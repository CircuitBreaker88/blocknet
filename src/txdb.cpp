...
bool CBlockTreeDB::LoadBlockIndexGuts(const Consensus::Params& consensusParams, std::function<CBlockIndex*(const uint256&)> insertBlockIndex)
{
    ...
    auto loadIndices = [&insertBlockIndex,&progress,totalprogress,estTotalBlocks,consensusParams,this](const std::vector<CDiskBlockIndex> & blocks, std::unordered_set<uint256, TXDBHasher> & invalidBlocks) {
    ...
    const auto fastSync = gArgs.GetBoolArg("-fastload", true);
    std::vector<CDiskBlockIndex> blocks;
    blocks.reserve(fastSync ? consensusParams.lastCheckpointHeight : std::min<int>(group, consensusParams.lastCheckpointHeight));
